...
## Appendix: type aliases and static arrays in var parameters (research conclusion, 2025-11)

Type aliases are fully transparent: `TData = array [0..1] of Integer` is interchangeable with `array [0..1] of Integer` everywhere, including `var` parameters. Alias resolution lives in `TypeAlias.Equals()`, array comparison in `ArrayType.Equals()`, compatibility in `IsCompatible()`; nested aliases resolve recursively and no special array-bounds handling is needed for by-ref parameters. The investigation found no defects. Full write-up: `docs/archive/arrays-type-compatibility-research.md`.
