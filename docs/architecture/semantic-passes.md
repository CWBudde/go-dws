> **Superseded (2026-09).** This describes a four-pass design with a shared `PassContext` that was never implemented. The analyzer is a single `Analyzer.Analyze` (`internal/semantic/analyzer.go`) that predeclares class types, registers top-level function signatures, then analyzes bodies. Kept as design input for the type-order-independence work in `PLAN.md` §3.2; do not treat it as a description of the code.
