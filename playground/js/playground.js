...
async function initWasm() {
  try {
    ...
    appendOutput(
      "❌ Failed to initialize DWScript WASM module\n" + error.message,
      "error"
    );
  }
}
...
function runCode() {
  ...
  try {
    ...
    if (result.success) {
      ...
      updateStats(
        `Execution time: ${result.executionTime}ms (total: ${executionTime}ms)`
      );
...
function updateEditorInfo() {
  ...
  document.getElementById(
    "editorInfo"
  ).textContent = `Line ${position.lineNumber}, Column ${position.column} | ${lineCount} lines`;
}
...
async function loadExample(key) {
  ...
  try {
    ...
    appendOutput(
      `❌ Failed to load example "${key}": ${error.message}\n`,
      "error"
    );
...
function applyTheme(theme, updateEditor = true) {
  ...
  if (updateEditor && editor) {
    monaco.editor.setTheme(
      theme === "dark" ? "dwscript-dark" : "dwscript-light"
    );
  }
}
