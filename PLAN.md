...
- **Type Aliases**: Full support for type alias declarations (`type TUserID = Integer`), semantic analysis, and runtime resolution
- **Const Declarations**: Constant declarations with immutability enforcement, semantic checking, and CLI integration
- **Built-in Functions**: Core runtime functions (Length, Copy, Pos, Delete, Insert) with comprehensive test coverage
- **Priority Array Functions**: High-value array manipulation functions integrated into standard library
- **Type Expression Parser**: Foundational infrastructure for parsing complex inline type expressions
- **Inline Function Pointer Types**: Support for inline function pointer syntax without requiring type aliases
- **Inline Array Types**: Dynamic and static array type expressions in variable/parameter declarations
- **Subrange Types**: Complete subrange type implementation with bounds checking and semantic validation
- **Lambdas/Anonymous Methods**: Full lambda expression support with both block and shorthand syntax
- **Units/Modules System**: Partial implementation of units/modules system for code organization (43 tasks complete)
- **Function/Method Pointers**: Complete function and method pointer support for callbacks and higher-order functions (26 tasks complete)
- **Bitwise Operators**: Bitwise AND, OR, XOR, SHL, SHR operators with parser, semantic, and interpreter support
- **External Function Registration / FFI**: Complete Foreign Function Interface with `RegisterFunction()` API, bidirectional type marshaling (primitives, arrays, maps), panic/error recovery to EHost exceptions, comprehensive test coverage (13 subtests for panic handling alone)
- **Helpers (Class/Record/Type)**: Complete helper type system extending existing types with additional methods without modification, supporting classes, records, and basic types (String, Integer, etc.) with method resolution priority and Self binding
- **DateTime Functions**: Comprehensive date/time functionality including current date/time access, formatting, parsing, arithmetic operations, and component extraction with full DWScript compatibility
- **Variant Type System**: Complete DWScript Variant type implementation for dynamic heterogeneous value storage, enabling array of const parameters, JSON support, and COM interop with boxing/unboxing, operators, comparisons, and built-in functions (VarType, VarIsNull, VarAsType, etc.)
- **JSON Support**: JSON parsing and serialization with Variant-based representation, supporting all JSON types (null, boolean, number, string, array, object) mapped to DWScript types where possible
- **Improved Error Messages and Stack Traces**: Enhanced error reporting with better messages, stack traces, source code snippets, and debugging information for improved developer experience
- **Contracts (Design by Contract)**: Complete DWScript contract system with preconditions (`require`), postconditions (`ensure`), `old` keyword for pre-execution values, and proper OOP inheritance semantics following Liskov substitution principle
- **Fixture Test Infrastructure**: Enhanced compatibility testing with DWScript reference suite, including program declarations, constructor parentheses optional syntax, zero-argument procedure calls, type meta-values, High/Low built-ins, and implicit type coercions
- **Advanced Property Modes**: Indexed properties (single/multi-index) with N-dimensional support, expression-based getters with Self binding, CLI test coverage (19 unit + 3 CLI tests)
- **Record Methods**: Parser, semantic analysis, and type system complete; interpreter method invocation pending
...
- [x] 9.21 Implement function call context type inference: **COMPLETED IN TASK 9.19**
  ...
  **PHASE 2: Handle overloaded functions** - **PARTIAL (2/3 complete)**
  - [x] 9.21.5: Overload detection ✅ **COMPLETE** (commit 6421334)
    ...
    - **Actual time**: 2 hours
  - [x] 9.21.6: Overload resolution ✅ **COMPLETE**
