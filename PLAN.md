...
### Study Phase
- [ ] 0.1 Clone/download the DWScript Delphi source code repository
- [ ] 0.2 Read through the main DWScript documentation and feature list
- [ ] 0.3 Identify and document the core DWScript components (tokenizer, parser, AST, semantic analyzer, runtime)
- [ ] 0.4 Map out the directory structure of the original DWScript codebase
- [ ] 0.5 Document the main entry points and data flow in DWScript compiler
- [ ] 0.6 Review DWScript's language specification and syntax rules
- [ ] 0.7 Study DWScript's test suite structure and coverage
- [ ] 0.8 Document differences between Pascal/Delphi and Go that will affect the port

### Go Project Setup
- [ ] 0.9 Initialize Go module: `go mod init github.com/cwbudde/go-dws`
- [ ] 0.10 Create initial directory structure:
  - [ ] `lexer/` - for tokenization
  - [ ] `parser/` - for parsing logic
  - [ ] `ast/` - for AST node definitions
  - [ ] `types/` - for type system
  - [ ] `interp/` - for interpreter/runtime
  - [ ] `cmd/dwscript/` - for CLI application
  - [ ] `testdata/` - for test scripts
- [ ] 0.11 Set up `.gitignore` with Go-specific entries
- [ ] 0.12 Create initial `README.md` with project description and goals
- [ ] 0.13 Set up `go.mod` and `go.sum` files

### CLI Scaffold
- [ ] 0.14 Add Cobra dependency: `go get -u github.com/spf13/cobra@latest`
- [ ] 0.15 Initialize Cobra CLI structure in `cmd/dwscript/`
- [ ] 0.16 Create root command with basic help text
- [ ] 0.17 Create `run` subcommand skeleton (accepts file path or `-e` for expression)
- [ ] 0.18 Add `version` subcommand
- [ ] 0.19 Implement file reading in `run` command (just echo content for now)
- [ ] 0.20 Add flag for inline code evaluation (`-e, --eval`)
- [ ] 0.21 Test CLI builds and runs: `go build ./cmd/dwscript`

### Version Control & CI
- [ ] 0.22 Initialize git repository: `git init`
- [ ] 0.23 Create initial commit with project structure
- [ ] 0.24 Set up GitHub repository (github.com/cwbudde/go-dws)
- [ ] 0.25 Push initial code to GitHub
- [ ] 0.26 Create `.github/workflows/test.yml` for GitHub Actions
- [ ] 0.27 Configure CI to run `go test ./...` on push/PR
- [ ] 0.28 Configure CI to run `go vet ./...`
- [ ] 0.29 Configure CI to run `golangci-lint` (optional but recommended)
- [ ] 0.30 Verify CI pipeline runs successfully

### Success Metrics Definition
- [ ] 0.31 Document the minimal viable compiler features (variables, expressions, control flow, functions)
- [ ] 0.32 Create a list of example scripts that should work by end of Stage 5
- [ ] 0.33 Define acceptance criteria for each stage
- [ ] 0.34 Set up a tracking document for feature parity with DWScript
