...
**Status**: Completed | **Priority**: Medium | **Source**: `backup/main-before-revised-phase-4-2026-05-08`
...
- The remaining useful old-main material was behavioral intent around environment restoration/synchronization for record methods and property getter/setter execution. It now survives as evaluator/runtime regression tests and targeted evaluator fixes only.
- `backup/main-before-revised-phase-4-2026-05-08` is archival history only. It can be retained briefly for audit/reference, but it should not be used as an implementation source and can be deleted once the team confirms no external references depend on it.

**Tasks**:

- [x] **X.1** Add focused regression tests for `SubString` and `StrToInt` old-main behavior so the already-ported fixes stay protected.
- [x] **X.2** Review old-main record method environment changes from `bbc58f19` against evaluator-owned `record_methods.go`; add tests for record method field sync, class var sync, static method scope restoration, and exception/early-return cleanup.
- [x] **X.3** Review old-main property environment changes from `04da5faa` against evaluator-owned `property_read.go` / `property_write.go`; add tests for method-backed getters/setters, expression-backed getters, indexed properties, and class-var synchronization.
- [x] **X.4** Review old-main string/conversion commits (`667cc23c`, `f1cb366a`) for missing edge-case tests in `internal/builtins`, `internal/bytecode`, and `internal/semantic`.
- [x] **X.5** After the salvage review is complete, document whether `backup/main-before-revised-phase-4-2026-05-08` can be retained only as archival history or deleted.
...
## Phase 4: Collapse To A Single Execution Engine ✅ **COMPLETED** (2026-03-10)

**Goal**: Delete the split-execution architecture where evaluator-owned AST execution bounced through interpreter callback interfaces, and leave one coherent runtime ownership model.

**Accomplishments**:

- Collapsed production construction to one public engine facade: `Interpreter` remains the shell/API boundary while `internal/interp/evaluator` owns AST execution.
- Made `ExecutionContext` the canonical owner of per-run mutable state, including environment, exception state, call stack, recursion depth context, and runtime execution metadata.
- Deleted the old callback-style execution surfaces: `CoreEvaluator`, `OOPEngine`, `DeclHandler`, `SetFocusedInterfaces()`, bridge-only files, and dead interpreter-side shadow evaluators.
- Normalized expression, statement, declaration, method/property dispatch, constructor, operator overload, helper, record, and type-cast execution under evaluator/runtime-owned paths, with shell-owned declaration/bootstrap exceptions explicitly allowlisted.
- Shrunk `internal/interp/contracts` to justified neutral coordination types and removed migration-era callback bundles and metadata escape hatches where canonical runtime/type-system APIs now exist.
- Resolved the remaining evaluator shim as an intentional minimal internal implementation handle rather than an unfinished deletion task.
- Removed the last AST-shaped external-function callback round-trip: evaluator now prepares runtime `[]Value` arguments, including var-parameter references, and the shell only performs value-level host invocation and FFI error handling.
- Unified runtime lifetime/refcount semantics across declarations, assignments, parameter binding, returns, pushed-scope cleanup, method pointers, interfaces, and collection copy helpers.
- Added architecture boundary tests that enforce the final import boundary, deleted callback surfaces, deleted shadow files, allowed `Interpreter.eval*` surface, and value-level external-function dispatch.

**Phases**: 4.0 Stabilization, 4.1 State Ownership, 4.2 Declaration Ownership, 4.3 Construction Collapse, 4.4 Execution Semantics, 4.5 Callback Deletion, 4.6 Metadata Cleanup, 4.7 Verification, 4.8 Refinement, 4.9 Residual Execution Ownership Cleanup, 4.10 Seam Clarification, 4.11 Neutral Boundary Finalization, 4.12 Lifetime/Refcount Consistency

**Key Docs**: [interp-evaluator-steady-state.md](docs/architecture/interp-evaluator-steady-state.md), [phase-4.10.2-seam-decisions.md](docs/architecture/phase-4.10.2-seam-decisions.md), [phase-4.10.5-interp-allowed-responsibilities.md](docs/architecture/phase-4.10.5-interp-allowed-responsibilities.md), [phase-4.11-neutral-boundary-audit.md](docs/architecture/phase-4.11-neutral-boundary-audit.md), [phase-4.12.1-lifetime-inventory.md](docs/architecture/phase-4.12.1-lifetime-inventory.md)

**Verification**:

- Focused Phase 4 architecture boundary suite passed, including the external-function no-AST-round-trip guard.
- Targeted external-function, FFI, var-parameter, and evaluator suites passed for the affected packages.
- A full `go test ./...` baseline was not claimed for this compaction; existing broader failures remain outside Phase 4 in frontend/parser/semantic test areas.
...
## Phase 6: Interpreter Technical Debt Cleanup ✅ **COMPLETED** (2026-05-08)

**Goal**: Address accumulated TODOs and technical debt in `internal/interp/...` discovered during Phase 3 work.

**Accomplishments**:

- Audited circular-import workaround TODOs and replaced stale assumptions with direct runtime/type-system implementations.
- Reviewed Phase 3 migration leftovers and re-scoped callback/AST-return cleanup to the Phase 4 architecture work where it belonged.
- Closed interpreter feature gaps for `is` inheritance checks, interface inheritance checks, enum constant coercion, record declarations, and class constant expressions in field initializers.
- Completed record `var`/by-ref handling for record variables, record fields, indexed record values, nested references, and record method calls.
- Added unit-owned function symbol tracking, unit-aware function registration, and qualified lookup verification so unit-qualified calls no longer borrow unrelated globals.
- Cleaned remaining small TODOs: JSON nil handling, static-array helper fallback coverage, function/method pointer type construction, and canonical type lookup from `getTypeByName`.

**Phases**: 6.1 Circular Import Workarounds, 6.2 Migration Leftovers Review, 6.3 Feature Implementation Gaps, 6.4 Var Parameter Handling, 6.5 Unit Symbol Tables, 6.6 Miscellaneous Cleanup

**Verification**:

- Targeted Phase 6.4-6.6 verification passed for affected interpreter, evaluator, units, interp/types, and internal/types packages.
- A full `go test ./...` baseline was not claimed as part of Phase 6; existing broader failures were observed outside these focused changes.
