...
func (p *Parser) parseClassDeclarationTraditional() *ast.ClassDecl {
...
//	end;
//
// PRE: cursor is at CLASS or TYPE; next token is class name IDENT
// POST: cursor is at END
...
func (p *Parser) parseClassParentAndInterfacesTraditional(classDecl *ast.ClassDecl) {
...
// parseClassParentAndInterfacesCursor parses optional parent class and interfaces from (...) (cursor mode).
// Can be called multiple times for syntax like: class abstract(TParent)
// Only updates classDecl if not already set to avoid overwriting previous parse
// PRE: cursor is at token before parent list (CLASS, ABSTRACT, or EXTERNAL)
// POST: cursor is at RPAREN if parentheses present; otherwise unchanged
...
// POST: curToken is END
func (p *Parser) parseClassDeclarationBodyTraditional(nameIdent *ast.Identifier) *ast.ClassDecl {
	builder := p.StartNode()
...
// parseClassDeclarationBodyCursor parses the body of a class declaration (cursor mode).
// Called after 'type Name = class' has already been parsed.
...
func (p *Parser) parseFieldDeclarationsTraditional(visibility ast.Visibility) []*ast.FieldDecl {
...
// parseFieldDeclarationsCursor parses a field declaration within a class (cursor mode).
// Syntax: FieldName: Type; or FieldName1, FieldName2, FieldName3: Type;
// The visibility parameter specifies the access level for this field.
// Returns a slice of FieldDecl nodes (one per field name) since DWScript supports
// comma-separated field names with a single type annotation.
// PRE: cursor is at first field name IDENT
// POST: cursor is at SEMICOLON
...
func (p *Parser) parseMemberAccessTraditional(left ast.Expression) ast.Expression {
	builder := p.StartNode()
...
func (p *Parser) parseClassConstantDeclarationTraditional(visibility ast.Visibility, isClassConst bool) *ast.ConstDecl {
	builder := p.StartNode()
...
// parseClassConstantDeclarationCursor parses a constant declaration within a class (cursor mode).
// Syntax: const Name = Value; or const Name: Type = Value;
// Also: class const Name = Value;
// The visibility parameter specifies the access level for this constant.
// The isClassConst parameter indicates if it was declared with 'class const'.
// PRE: cursor is at constant name IDENT
// POST: cursor is at SEMICOLON
