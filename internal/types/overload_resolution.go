...
func SignaturesEqual(sig1, sig2 *FunctionType) bool {
...
// SignatureDistance calculates the "distance" between argument types and parameter
...
//	 2 = Conversion from Variant; reference/array conversions may add penalties
//	 4 = Boxing into Variant
//	-1 = Incompatible (no conversion possible)
//
// This is used to rank overload candidates when multiple overloads could match
// the provided arguments.
func SignatureDistance(argTypes []Type, signature *FunctionType) int {
	...
	for i, argType := range argTypes {
		...
		if i < len(signature.StrictParams) && signature.StrictParams[i] {
			if !IsIdentical(argType, paramType) {
				return -1
			}
			continue
		}
...
// typeDistance calculates the conversion distance between two
...
func typeDistance(from, to Type) int {
	...
	if fromArray, ok := from.(*ArrayType); ok {
		if toArray, ok := to.(*ArrayType); ok {
	...
	if fromMeta, ok := from.(*ClassOfType); ok {
		if toMeta, ok := to.(*ClassOfType); ok {
...
func classDistance(from, to *ClassType) int {
	if from == nil || to == nil {
		return -1
	}
...
func arrayDistance(from, to *ArrayType) int {
	if from == nil || to == nil {
		return -1
	}
	...
	// into a plain Variant parameter (which costs 4): cap it at 2 so an
	// "array of Variant" parameter wins for array arguments.
	if elemDist > 2 && GetUnderlyingType(to.ElementType).TypeKind() == "VARIANT" {
		elemDist = 2
	}
...
//
// Parameters:
//   - candidates: Function types representing overloaded functions (non-function types are skipped)
//   - argTypes: Actual argument types from the call site
//
// Returns:
//   - The index of the best matching candidate, or -1 on error
...
//  4. Prefer a sole non-variadic candidate in a tie; otherwise report ambiguity
//  5. If no compatible candidates, return no match error
func ResolveOverload(candidates []Type, argTypes []Type) (int, error) {
	if len(candidates) == 0 {
		return -1, fmt.Errorf("no overload candidates provided")
	}

	// Single candidate - no resolution needed
	if len(candidates) == 1 {
		// Still need to verify compatibility
		funcType, ok := candidates[0].(*FunctionType)
		if !ok {
			return -1, fmt.Errorf("candidate is not a function type")
		}
		dist := SignatureDistance(argTypes, funcType)
		if dist < 0 {
			return -1, fmt.Errorf("no matching overload for argument types")
		}
		return 0, nil
	}
	...
	for index, candidate := range candidates {
		funcType, ok := candidate.(*FunctionType)
		if !ok {
	...
	if len(compatible) == 0 {
		return -1, fmt.Errorf("no matching overload for argument types: %s", formatArgTypes(argTypes))
	}
	...
	for _, c := range compatible {
		if c.distance == minDist {
			bestMatches = append(bestMatches, c.index)
		}
	}
	...
	for _, match := range bestMatches {
		if ft, ok := candidates[match].(*FunctionType); ok && !ft.IsVariadic {
	...
	return -1, fmt.Errorf("ambiguous overload call: %d candidates with equal distance %d for argument types: %s",
		len(bestMatches), minDist, formatArgTypes(argTypes))
}

// formatArgTypes formats argument types for error messages
func formatArgTypes(argTypes []Type) string {
	if len(argTypes) == 0 {
		return "()"
	}
