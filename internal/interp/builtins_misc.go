...
// builtinSetLengthValue implements the SetLength() built-in function for evaluated values.
// This is kept for compatibility but may not be used.
func (i *Interpreter) builtinSetLengthValue(args []Value) Value {
	if len(args) != 2 {
		return i.newErrorWithLocation(i.currentNode, "SetLength() expects exactly 2 arguments, got %d", len(args))
	}

	// First argument must be a dynamic array or string (passed by reference)
	firstArg := args[0]

	// Handle var parameters (ReferenceValue)
	var refVal *ReferenceValue
	if ref, isRef := firstArg.(*ReferenceValue); isRef {
		refVal = ref
		// Dereference to get the actual value
		derefVal, err := ref.Dereference()
		if err != nil {
			return i.newErrorWithLocation(i.currentNode, "failed to dereference variable: %s", err)
		}
		firstArg = derefVal
	}
	...
	// Handle arrays
	if arrayVal, ok := firstArg.(*ArrayValue); ok {
		// Check that it's a dynamic array
		if arrayVal.ArrayType == nil {
			return i.newErrorWithLocation(i.currentNode, "array has no type information")
		}
	...
	// Handle strings
	if strVal, ok := firstArg.(*StringValue); ok {
		currentLength := len(strVal.Value)
		...
		if newLength < currentLength {
			...
			// Extend the string with null characters (or spaces)
			// DWScript typically extends with null characters
			newStr = strVal.Value + string(make([]byte, newLength-currentLength))
		}
	...
	return i.newErrorWithLocation(i.currentNode, "SetLength() expects array or string as first argument, got %s", firstArg.Type())
}
